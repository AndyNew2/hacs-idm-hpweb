...
idmServiceModeIndicatorIndex = (
    5  # index in idmInputOutputDescr where service mode is indicated
)
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if startPos == -1:
                    ...
                    if startPos == -1:
                        _LOGGER.warning(
                            "Identification string not found, wrong frame, or unknown language! We still try to extract data."
                        )
                        startPos = (
                            0  # start at begin of frame to try to extract data anyway
                        )

                startPos = txt.find(
                    idmInputOutputDescr[0], startPos
                )  # idenifier for extra data
                if startPos == -1:
                    _LOGGER.warning(
                        "Wrong answer received, no extra data can be extracted!"
                    )
                ...
                if afterPos == -1:
                    _LOGGER.warning(
                        "Wrong answer received, no further values can be extracted!"
                    )
                    return answerData
                ...
                for i in range(1, len(idmInputOutputDescr)):
                    ...
                    if i >= idmServiceModeIndicatorIndex:
                        afterPos = txt.find(
                            idmInputOutputDescr[i], startPos
                        )  # in service mode or PV data is much later, we cannot optimize here
                    else:
                        afterPos = txt.find(
                            idmInputOutputDescr[i],
                            startPos,
                            startPos + idmReadAheadBlock,
                        )
                    if (afterPos == -1) and (i < idmServiceModeIndicatorIndex):
                        _LOGGER.warning(
                            "Wrong order of input/output sections, try to search again ..."
                        )
                        afterPos = txt.find(idmInputOutputDescr[i], 0)
                        if afterPos == -1:
                            _LOGGER.warning(
                                "Cannot find section %s, stop parsing for this section.",
                                idmInputOutputDescr[i],
                            )
                            break
                    ...
                    keyStr = ""
                    while True:
                        (valStr, keyStr, afterPos) = (
                            extractParameterInputOutputInSectionWithKey(txt, startPos)
                        )
                        ...
                        if v is None:
                            ...
                            keyStr = (
                                keyStr + "#" + str(i - 1)
                            )  # to differenciate multiple same keys in different sections
                            v = self.idmSensorDefn.get(keyStr)
                        if v is None:
                            _LOGGER.debug(
                                "Key %s not found in sensor definitions. Will be ignored.",
                                keyStr,
                            )
                        else:
                            ...
                            if v in (
                                ...
                                serviceMode = (
                                    True  # detected Service mode, add further section
                                )
                                _LOGGER.debug(
                                    "Service mode detected, enabling service mode parameters."
                                )
                            if len(keyStr) <= 5:
...
def extractParameterInputOutputInSectionWithKey(txt, startPos):
    ...
    newPos = txt.find(
        idmSectionDelimiter, startPos, entryStart
    )  # in case there is a section delimiter before entry start, we have reached end of section
    if newPos != -1:
    ...
    newPos = entryStart + len(
        idmKeyIntro
    )  # position for key is straight after entry begin
    ...
    if (keyStr == "") or (keyStr == " "):
        ...
        newPos = endPos + len(
            idmKeyEnding
        )  # description ending is the same as value intro, therefore set newPos to it
    else:
        endPos += len(
            idmKeyEnding
        )  # we need to search for value intro, but make sure we are after key ending
